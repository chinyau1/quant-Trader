#!/usr/bin/env bash
# Quant-Trader 在线一键部署（Ubuntu / Debian）
#
# 在新服务器上执行：
...
#
# 已部署服务器请用 deploy/update.sh，不会清除 data/logs/.env。
#
# 参数（curl 管道需用 bash -s）：
...
ensure_env() {
    ...
    if ! grep -Eq '^(DEEPSEEK_API_KEY|OPENAI_API_KEY|GLM_API_KEY)="?[^"]+' "$env_file"; then
        ...
        log "      重跑时传入: sudo env DEEPSEEK_API_KEY='sk-xxx' bash deploy/deploy.sh"
    fi
}
...
ensure_dsh_im() {
    ...
    log '在远程 baymax-dsh 安装/确保 dsh-im'
    cd "$PROJECT_DIR"
    if ! docker ps --format '{{.Names}}' | grep -qx baymax-dsh; then
        log "baymax-dsh 未运行，跳过 dsh-im"
        return
    fi
    docker exec baymax-dsh dsh plugin --profile web add -w @xmanrui/dsh-im \
...
show_completion() {
    ...
    echo "  4. 更新代码（不清除数据）: cd $PROJECT_DIR && sudo bash deploy/update.sh"
    echo "========================================================================="
    echo ""
}
